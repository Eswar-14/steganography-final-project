# import the liabraries files
import cv2
import os
import hashlib
# Specify the path to the input image
image_path = input(r' Enter the input image path :: ')
# Read the input image 
img = cv2.imread(image_path)
# Check weather the image is loaded or not
if img is None:
  print("Image not found. Check the file path and make sure the image exists.")
  exit()
# Get the dimensions of the image
height, width, channels = img.shape
# Getting the secret mesaage
...
for i in range(256):
  ...
  if m >= width:
    m = 0
    n += 1
  # If the row index exceeds the image height, stop encoding (message too large forimage)
  if n >= height:
